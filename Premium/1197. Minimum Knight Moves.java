    ...
    public int minKnightMoves(int x, int y) {
        ...
        while(!q.isEmpty()){
            ...
            for(int j=0;j<size;j++){
                ...
                if(temp[0]==x && temp[1]==y) return steps;
...
Given the coordinate of the target as (x, y)(x,y), the number of cells covered by the circle that is centered at point (0, 0)(0,0) and reaches the target point is roughly \big(\max(|x|, |y|)\big) ^ 2(max(∣x∣,∣y∣)) 
2
 .

Time Complexity: O\bigg(\big(\max(|x|, |y|)\big) ^ 2\bigg)O((max(∣x∣,∣y∣)) 
2
 )

Due to the nature of BFS, before reaching the target, we will have covered all the neighborhoods that are closer to the start point. The aggregate of these neighborhoods forms a circle, and the area can be approximated by the area of a square with an edge length of \max(2|x|, 2|y|)max(2∣x∣,2∣y∣). The number of cells within this square would be \big(\max(2|x|, 2|y|)\big) ^ 2(max(2∣x∣,2∣y∣)) 
2
 .

Hence, the overall time complexity of the algorithm is O\bigg(\big(\max(2|x|, 2|y|)\big) ^ 2 \bigg) = O\bigg(\big(\max(|x|, |y|)\big) ^ 2 \bigg)O((max(2∣x∣,2∣y∣)) 
2
 )=O((max(∣x∣,∣y∣)) 
2
 ).

Space Complexity: O\bigg(\big(\max(|x|, |y|)\big) ^ 2\bigg)O((max(∣x∣,∣y∣)) 
2
 )

We employed two data structures in the algorithm, i.e. queue and set.

At any given moment, the queue contains elements that are situated in at most two different layers (or levels). In our case, the maximum number of elements at one layer would be 4 \cdot \max(|x|, |y|)4⋅max(∣x∣,∣y∣), i.e. the perimeter of the exploration square. As a result, the space complexity for the queue is O\big( \max(|x|, |y|) \big)O(max(∣x∣,∣y∣)).

As for the set, it will contain every elements that we visited, which is \big(\max(|x|, |y|)\big) ^ 2(max(∣x∣,∣y∣)) 
2
  as we estimated in the time complexity analysis. As a result, the space complexity for the set is O\bigg(\big(\max(|x|, |y|)\big) ^ 2\bigg)O((max(∣x∣,∣y∣)) 
2
 ).

To sum up, the overall space complexity of the algorithm is O\bigg(\big(\max(|x|, |y|)\big) ^ 2\bigg)O((max(∣x∣,∣y∣)) 
2
...
This can be proved mathematically. Suppose that the distance between the start and target points is dd, the exploration scope covered by the unidirectional BFS will be a circle whose area is \pi \cdot d^2π⋅d 
2
 . On the other hand, with the bidirectional BFS, the exploration scope will be two smaller circles whose total area is 2 \cdot \pi (\frac{d}{2})^2 = \pi \cdot \frac{d^2}{2}2⋅π( 
2
d
​
 ) 
...
First of all, given the target's coordinate, (x, y)(x,y), then the area that is covered by the two exploratory circles of the bidirectional BFS will be {\max(|x|, |y|) ^ 2}/{2}max(∣x∣,∣y∣) 
2
 /2.
...
Reducing the scope of exploration by half does speed up the algorithm. However, it does not change the time complexity of the algorithm which remains O\bigg(\big(\max(|x|, |y|)\big) ^ 2\bigg)O((max(∣x∣,∣y∣)) 
2
 ).
Space Complexity: O\bigg(\big(\max(|x|, |y|)\big) ^ 2\bigg)O((max(∣x∣,∣y∣)) 
2
...
In exchange for reducing the search scope, we double the usage of data structures compared to the unidirectional BFS. Similarly to the time complexity, multiplying the required space by two does not change the overall space complexity of the algorithm which remains O\bigg(\big(\max(|x|, |y|)\big) ^ 2\bigg)O((max(∣x∣,∣y∣)) 
2
 ).
...
\text{dfs}(x, y) = \min\big(\text{dfs}(|x-2|, |y-1|), \text{dfs}(|x-1|, |y-2|)\big) + 1dfs(x,y)=min(dfs(∣x−2∣,∣y−1∣),dfs(∣x−1∣,∣y−2∣))+1
...
Time Complexity: O(|x \cdot y|)O(∣x⋅y∣)
...
In the algorithm, we restrict the exploration to the first quadrant of the board. Therefore, in the worst case, we will explore all of the cells between the origin and the target in the first quadrant. In total, there are |x \cdot y|∣x⋅y∣ cells in a rectangle that spans from the origin to the target. As a result, the overall time complexity of the algorithm is O(|x \cdot y|)O(∣x⋅y∣).

Space Complexity: O(|x \cdot y|)O(∣x⋅y∣)
...
Secondly, due to the application of memoization technique, we will keep all the intermediate results in the memory for reuse. As we have seen in the above time complexity analysis, the maximum number of intermediate results will be O(|x \cdot y|)O(∣x⋅y∣).

To sum up, the overall space complexity of the algorithm is O(|x \cdot y|)O(∣x⋅y∣), which is dominated by the memoization part.
